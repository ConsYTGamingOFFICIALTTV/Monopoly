...
const Monopoly = {
    ...
    load: (
        ...
        mySound = new sound("https://s3-us-west-2.amazonaws.com/s.cdpn.io/217233/featured.mp3");
        myFaliure = new sound("http://soundbible.com/grab.php?id=1830&type=mp3");
        myMusic = new sound("http://www.mariomayhem.com/downloads/sounds/Super_Mario_Land_sfx/powerup.wav");
        myCheer = new sound("https://s3-us-west-2.amazonaws.com/s.cdpn.io/217233/researchComplete.mp3");
        myJail = new sound("http://www.noproblo.dayjo.org/ZeldaSounds/MC/MC_Cucco6.wav");
        myMystery = new sound("http://www.noproblo.dayjo.org/ZeldaSounds/LOZ/LOZ_Secret.wav");
        mySpyrit = new sound("http://www.noproblo.dayjo.org/ZeldaSounds/BOTW/BOTW_Fanfare_SpiritOrb.wav");
        ...
        const settings = {
            ...
            jailedTurn: 3,
            rentMultiplier: 1.5, //For slower game
            taxMultiplier: 1.5 //For slower game
        };
        ...
        class Board extends GameAsset {
            ...
            setup() {
                ...
                logo.className = 'logo';
                logo.innerText = 'MONOPOLY HARRY POTTER LIMITED EDITION';
              this.settingsDisplay.append(logo);
              const Text = document.createElement('p');
                Text.className = 'Text';
                Text.innerText = 'Rules: This Harry Potter Limited Edition Should not be replicated. if you are a coder, please contact: harshavardhan.parthiban@gmail.com before replicating. the Chance and Community Chest cards face down on their allotted spaces on the board. Each player will be given one token to represent them while traveling around the board. Each player is given $1500 by the computer in order to play. There will be a banker, and the bot will be the banker all the way. HOW TO PLAY? Starting with the Banker, each player throws the dice. The player with the highest total starts the play. Then each player places his token on the corner marked "GO", and throw the dice and move his the number of spaces indicated by the dice. The tokens remain on the spaces occupied and proceed from that point on the players next turn. Two or more tokens may rest on the same space at the same time. If one or both dice roll off the board, or land on or lean against a card deck, the roll is invalid. Roll them again. Depending on the space the token reaches, you may buy the property, or be obliged to pay rent, pay taxes, draw a Chance or Community Chest card, Go To Jail, or etc... If you throw doubles, you move your token and are subject to any privileges or penalties pertaining to the space on which you land. Then you can throw again and move your token as before. But watch out, if you ever throw doubles three times in succession, you have to go immediately to jail.';
                this.settingsDisplay.append(Text);
        ...
        class Space extends GameAsset {
            ...
            updateSpaceDetailDisplay() {
                ...
                iconContainer.style['border'] = `10px solid ${typeof (this.color) === 'undefined' ? 'white' : this.color}`;
                let detailIcon = '';
        ...
        class Jail extends Space {
            ...
            resolve(player) {
                console.log(`${player.name} is found by a small child`);
                myJail.play();
                player.state = 'caught';
                player.stateTurn = settings.jailedTurn;
                const message = 'is found by a small child';
                const button = UIHelpers.dialog(player, message).get('accept');
                button.addEventListener('click', () => {
                    game.finishTurn();
                });
        ...
        class Property extends Space {
            constructor(param) {
                ...
                if (this.row === 1) {
                    this.boardSpaceWrapper.style['border-bottom'] = `10px solid ${this.color}`;
                }
                if (this.row === 11) {
                    this.boardSpaceWrapper.style['border-top'] = `10px solid ${this.color}`;
                }
                if (this.column === 1) {
                    this.boardSpaceWrapper.style['border-right'] = `10px solid ${this.color}`;
                }
                if (this.column === 11) {
                    this.boardSpaceWrapper.style['border-left'] = `10px solid ${this.color}`;
                }
        ...
        class Game {
            ...
            play() {
                ...
                if (player.state !== 'bankrupt') {
                    ...
                    if (player.state === 'playing') {
                        ...
                        const buttonText = player.stateTurn > 1 ? 'Skip Turn' : 'escape';
            ...
            waitrolldice(){
                ...
                let icon = '❔';
                const button = UIHelpers.dialog(player, message, [new Option('accept', buttonText)], icon).get('accept');
                        button.addEventListener('click', () => {
                            this.rollDice();
                        });  
            };
        ...
        class GameHelpers {
            ...
            static setupPlayers() {
                game.addPlayer({ name: 'Aksitha', money: settings.startingAmount, icon: '👩' });
                game.addPlayer({ name: 'Shenaz', money: settings.startingAmount, icon: '👧' });
                game.addPlayer({ name: 'Harsha', money: settings.startingAmount, icon: '👦' });
                game.addPlayer({ name: 'Luigui', money: settings.startingAmount, icon: '👦' });
            }
